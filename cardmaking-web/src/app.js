...
const stampOptions = [
  { id: "none", label: "不贴邮票", color: "#fdf7ee", accent: "#b64a35", title: "邮票", mark: "空" },
  { id: "panda", label: "熊猫成都", color: "#e7f3ec", accent: "#2f7d68", title: "成都", mark: "熊猫" },
  { id: "bamboo", label: "竹影春日", color: "#f4f0d8", accent: "#6f8a39", title: "春日", mark: "竹" },
  { id: "wuhou", label: "锦官红墙", color: "#f9e7df", accent: "#b64a35", title: "锦官城", mark: "蜀" },
  { id: "poem", label: "诗笺留念", color: "#eaf1f3", accent: "#4b6b78", title: "诗意", mark: "笺" }
];
...
function renderStamp(stamp) {
  ...
  return `
    <span class="stamp stamp-art" style="--stamp-bg:${stamp.color}; --stamp-accent:${stamp.accent};">
      <span class="stamp-title">${stamp.title}</span>
      <span class="stamp-mark">${stamp.mark}</span>
    </span>
  `;
}
...
function drawBack(ctx, card, x, y, w, h) {
  ...
  ctx.lineWidth = 3;
  drawStamp(ctx, stamp, x + w - 206 * scale, y + 50 * scale, 132 * scale, 96 * scale);
...
function drawStamp(ctx, stamp, x, y, w, h) {
  ctx.save();
  if (stamp.id === "none") {
    ctx.strokeStyle = "rgba(182,74,53,0.58)";
    ctx.lineWidth = 3;
  ...
  ctx.fillStyle = stamp.accent;
  ctx.font = `700 ${Math.round(h * 0.18)}px Microsoft YaHei UI, sans-serif`;
  ctx.textAlign = "center";
  ctx.fillText(stamp.title, x + w / 2, y + h * 0.24);
  ctx.font = `700 ${Math.round(h * 0.36)}px STKaiti, KaiTi, serif`;
  ctx.fillText(stamp.mark, x + w / 2, y + h * 0.68);
  ctx.restore();
}
