...
const stampOptions = [
  { id: "none", label: "不贴邮票" },
  { id: "panda", label: "熊猫竹影", image: "assets/stamps/stamp-panda-bamboo.png" },
  { id: "cottage", label: "草堂庭院", image: "assets/stamps/stamp-cottage-garden.png" },
  { id: "wuhou", label: "红墙翠竹", image: "assets/stamps/stamp-red-wall-bamboo.png" },
  { id: "bonsai", label: "窗影盆景", image: "assets/stamps/stamp-window-bonsai.png" }
];
...
function ensureCardControls(card) {
  ...
  card.stampId = card.stampId || "none";
  return card;
}
...
function icon(name) {
  const paths = {
    ...
    trash: '<path d="M3 6h18"/><path d="M8 6V4h8v2"/><path d="M7 6l1 15h8l1-15"/><path d="M10 11v6"/><path d="M14 11v6"/>'
  };
      ...
      <div class="postcard-list">
        ...
        ${state.cards.map((item) => `
          <div class="postcard-item ${item.id === card.id ? "active" : ""}" data-card-id="${item.id}">
            ...
            <button class="delete-card-btn" data-delete-card="${item.id}" title="删除这个作品" aria-label="删除 ${escapeAttr(item.title)}">${icon("trash")}</button>
          </div>
        `).join("")}
      </div>
    </aside>
      ...
      <section class="crop-panel">
        ...
        <div class="crop-stage" style="--crop-w:${boxW}px; --crop-h:${boxH}px;">
          ...
          <div class="crop-frame">
            <span class="crop-resize-handle" data-crop-resize title="拖动调整照片大小"></span>
          </div>
        </div>
        <div class="crop-controls">
          <div class="simple-help">
            <strong>直接拖动照片调整裁剪</strong>
            <span>拖动照片移动位置；拖右下角圆点改变照片大小。</span>
          </div>
          <div class="quick-actions">
            <button class="tool-btn" data-crop-scale="-0.1">缩小</button>
            <button class="tool-btn" data-crop-reset>重置</button>
            <button class="tool-btn" data-crop-scale="0.1">放大</button>
          </div>
  ...
  return `
    ...
    <div class="front-copy" style="--front-x:${card.frontLayout.x}px; --front-y:${card.frontLayout.y}px; --front-gap:${card.frontLayout.gap}px; --text-theme:${theme.css}; --text-weight:${theme.weight}; --text-tracking:${theme.tracking};">
      ${card.front.map((line, index) => `
        <div class="${index === 1 ? "main" : ""} draggable-text" data-drag-text="${index}" style="font-size:${line.size}px; --line-index:${index}; --line-x:${line.x}px; --line-y:${line.y}px;">${escapeHtml(line.text)}</div>
      `).join("")}
    </div>
  `;
}
...
function renderBack(card) {
  ...
  const theme = textThemes[card.textTheme] || textThemes.hand;
  const stamp = stampOptions.find((item) => item.id === card.stampId) || stampOptions[0];
  return `
    <div class="back-face">
...
function renderStamp(stamp) {
  if (stamp.id === "none") {
    return '<span class="stamp stamp-empty">贴邮票处</span>';
  }
  return `
    <span class="stamp stamp-art">
      <img class="stamp-image" src="${stamp.image}" alt="${stamp.label}">
    </span>
  `;
}
  ...
  return `
    <aside class="inspector">
      ...
      <div class="panel-section">
        ...
        <div class="simple-help">
          <strong>移动文字</strong>
          <span>在明信片正面直接拖动某一行文字即可调整位置。</span>
        </div>
      </div>
        ...
        <div class="control-group">
          ...
          <div class="stamp-picker">
            ${stampOptions.map((stamp) => `
              <button class="stamp-choice ${card.stampId === stamp.id ? "active" : ""}" data-stamp-id="${stamp.id}" title="${stamp.label}">
                ${renderStamp(stamp)}
      ...
      <div class="panel-section">
        ...
        <div class="control-group" style="margin-top:14px">
          <h3>照片调整</h3>
          <div class="simple-help">
            <span>在明信片正面直接拖动照片调整构图。</span>
          </div>
          <div class="quick-actions">
            <button class="tool-btn" data-photo-scale="-0.08">缩小</button>
            <button class="tool-btn" data-photo-reset>重置</button>
            <button class="tool-btn" data-photo-scale="0.08">放大</button>
          </div>
        </div>
      </div>
...
function bindEvents() {
  ...
  document.querySelectorAll("[data-front-text]").forEach((input) => {
    input.addEventListener("input", () => {
      const index = Number(input.dataset.frontText);
      activeCard().front[index].text = input.value;
      updateFrontLine(index);
    });
  });

  document.querySelectorAll("[data-front-size]").forEach((input) => {
    input.addEventListener("input", () => {
      const index = Number(input.dataset.frontSize);
      activeCard().front[index].size = Number(input.value);
      updateFrontLine(index);
    });
  });
  ...
  document.querySelectorAll("[data-stamp-id]").forEach((button) => {
    button.addEventListener("click", () => {
      activeCard().stampId = button.dataset.stampId;
      state.side = "back";
      render();
    });
  });

  document.querySelectorAll("[data-photo-scale]").forEach((button) => {
    button.addEventListener("click", () => {
      const card = activeCard();
      card.photoAdjust.scale = Number(clamp(card.photoAdjust.scale + Number(button.dataset.photoScale), 0.35, 3.2).toFixed(2));
      state.side = "front";
      render();
    });
  });
  ...
  document.querySelector("[data-photo-reset]")?.addEventListener("click", () => {
    activeCard().photoAdjust = { scale: 1, x: 0, y: 0 };
    state.side = "front";
    render();
  });
...
function updateFrontLine(index) {
  if (state.side !== "front") return;
  const line = activeCard().front[index];
  const node = document.querySelector(`[data-drag-text="${index}"]`);
  if (!node || !line) return;
  node.textContent = line.text;
  node.style.fontSize = `${line.size}px`;
}
...
function updateBackPreview() {
  ...
  if (!postcard) return;
  postcard.innerHTML = renderBack(activeCard());
}
    ...
    target.addEventListener("pointerdown", (event) => {
      ...
      const onMove = (moveEvent) => {
        ...
        if (isPhoto) {
          ...
          const limit = frontTextDragLimit(card);
          line.x = Math.round(clamp(start.x + dx, -limit.x, limit.x));
          line.y = Math.round(clamp(start.y + dy, -limit.y, limit.y));
...
function bindCropModal() {
  ...
  document.querySelectorAll("[data-crop-scale]").forEach((button) => {
    button.addEventListener("click", () => {
      state.cropDraft.scale = Number(clamp(state.cropDraft.scale + Number(button.dataset.cropScale), 0.6, 2.6).toFixed(2));
      render();
    });
  });

  const resizeHandle = document.querySelector("[data-crop-resize]");
  if (resizeHandle) {
    resizeHandle.addEventListener("pointerdown", (event) => {
      ...
      const startX = event.clientX;
      const startY = event.clientY;
      const startScale = state.cropDraft.scale;
      resizeHandle.setPointerCapture(event.pointerId);
      resizeHandle.classList.add("is-dragging");

      const onMove = (moveEvent) => {
        const delta = (moveEvent.clientX - startX + moveEvent.clientY - startY) / 280;
        state.cropDraft.scale = Number(clamp(startScale + delta, 0.6, 2.6).toFixed(2));
        const cropImage = document.querySelector("[data-crop-image]");
        if (cropImage) cropImage.style.setProperty("--crop-scale", state.cropDraft.scale);
      };
...
async function exportPostcard() {
  try {
    ...
    await drawBack(ctx, card, 0, size.h + gap, size.w, size.h);
...
async function drawFront(ctx, card, x, y, w, h) {
  ...
  card.front.forEach((line, index) => {
    ...
    const ascent = metrics.actualBoundingBoxAscent || line.size * scale * 0.82;
    const textTop = baseY + index * card.frontLayout.gap * scale + line.y * scale;
    ctx.fillText(line.text, centerX + line.x * scale, textTop + 2 * scale + ascent);
  });
  ctx.shadowColor = "transparent";
}

async function drawBack(ctx, card, x, y, w, h) {
  const theme = textThemes[card.textTheme] || textThemes.hand;
  const stamp = stampOptions.find((item) => item.id === card.stampId) || stampOptions[0];
  ...
  ctx.lineWidth = 3;
  await drawStamp(ctx, stamp, x + w - 244 * scale, y + 50 * scale, 170 * scale, 108 * scale);
  ...
  const messageBlockH = lines.length * lineHeight + (card.signature ? 64 * scale : 0);
  let textY = y + h / 2 - messageBlockH / 2 + 26 * scale + card.backLayout.y * scale;
  lines.forEach((line) => {
...
async function drawStamp(ctx, stamp, x, y, w, h) {
  ctx.save();
  if (stamp.id === "none") {
    ctx.strokeStyle = "rgba(182,74,53,0.58)";
    ctx.lineWidth = 3;
    ...
    ctx.fillStyle = "#b64a35";
    ctx.font = `${Math.round(22 * (w / 132))}px Microsoft YaHei UI, sans-serif`;
    ctx.textAlign = "center";
    ctx.fillText("贴邮票处", x + w / 2, y + h * 0.6);
    ctx.restore();
    return;
  }
  try {
...
function frontTextDragLimit(card) {
